...
void satellite_receiver_task(void *pvParameters)
{
    ...
    const portBASE_TYPE satellite_block_time = 130UL / portTICK_RATE_ONE_THIRD_MS;	        // One frame each 22 ms. This causes an error message if no valid rc-signal is detected. 
    const portBASE_TYPE satellite_block_time_sync = 10UL / portTICK_RATE_ONE_THIRD_MS;		// One frame each 22 ms
	...
	{ 
        ...
        bytes_read = freertos_usart_serial_read_packet(spectrum_usart, satellite_receive_buffer, 
                                                        SATELLITE_MESSAGE_LENGTH, 
                                                        satellite_block_time);

		satellite_decode_signal(satellite_receive_buffer, &satellite_data);

        /*If not 16 bytes are read something is wrong */
        if ((satellite_data.connection_ok == 0) || (bytes_read != SATELLITE_MESSAGE_LENGTH))
        {
            /*error!*/
